...
// 安装指定版本echarts:npm i echarts@5.4.3
// 安装指定版本echarts-gl:npm i echarts-gl@2.0.9

// 一、引入echarts-gl
import "echarts-gl"; //echarts 3D插件
// 二、局部引入echarts
import * as echarts from "echarts";
// 三、引入世界json文件，里面存储了很多经纬度
import world from "./json/world.json";
const worldData: any = world; 
// 四、调用echarts中的registerMap方法去实例化数据
echarts.registerMap("world", worldData);

// 配置项在这个网页中复制：https://www.isqqw.com/viewer?id=37071
const handleSearch = () => {
    // 2 初始化echarts实例 获取dom元素
    ...
    function drawEarth() {
        ...
        for (let i = 0; i < 50; i++) {
            let rdata:any = rodamData();
            option.series[1].data = option.series[1].data.concat(rdata);
        }
